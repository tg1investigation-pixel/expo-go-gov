...
export default function SearchResultsScreen({ route, navigation }) {
  const { results, total, searchType, query } = route.params;

  const renderResultItem = ({ item, index }) => {
    return (
      <TouchableOpacity
        style={styles.resultCard}
        onPress={() => navigation.navigate('Detail', { record: item, searchType })}
        activeOpacity={0.7}
      >
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <FlatList
          ...
          keyExtractor={(item, index) => `${item.id || item._id || index}-${item.table_name || ''}`}
